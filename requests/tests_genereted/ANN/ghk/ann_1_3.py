...
def test_digit_recognition_accuracy(model: tf.keras.Model, test_set):
    ...
    # Normalize and flatten the input data (as per the implementation)
    normalized_images = images / 255.0
    flattened_images = normalized_images.reshape(-1, 28 * 28)

    # Use the model directly for prediction since we have it available in this test context
    predictions = np.array([int(np.argmax(model.predict(image.reshape(1, -1)))) for image in flattened_images])
    
    # Calculate accuracy using scikit-learn's metric function (more reliable than manual calculation)
    actual_accuracy = accuracy_score(expected_labels, predictions)

    assert actual_accuracy > 0.95
