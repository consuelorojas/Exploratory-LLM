...
def test_digit_recognition_accuracy(model: tf.keras.Model, test_set):
    ...
    # Initialize classifier instance with loaded model
    ...
    predicted_labels = ClassifyDigits()(x_test)
    
    # Calculate accuracy by comparing true and predicted labels
    correct_predictions = sum(1 for pred, actual in zip(predicted_labels, y_test) if pred == actual)
