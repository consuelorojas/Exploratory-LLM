...
class TestDigitClassification:
    ...
    def test_classification_accuracy(self, classify_digits):
        ...
        for filename in os.listdir(TEST_IMAGES_DIR):
            ...
            predicted_digit = classify_digits(np.array([image_array]))[0]
    ...
    def test_invalid_image_path(self, classify_digits):
        with pytest.raises(FileNotFoundError):
            image_array = np.array(PIL.Image.open("invalid_image.png").convert('L').resize((28, 28)))
            classify_digits(np.array([image_array]))
