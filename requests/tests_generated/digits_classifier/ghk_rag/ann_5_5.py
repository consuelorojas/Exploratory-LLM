...
def test_recognize_digits_bulk(classifier):
    ...
    assert len(predictions) == 10, "Expected exactly 10 images to be tested"

def test_accuracy(classifier):
    """
    Test that the classifier recognizes over 95% of input as digits correctly.
    ...
    correct_count = sum(1 for pred, actual in zip(predictions, y_test[:100]) if pred == actual)
    accuracy = (correct_count / len(y_test[:100])) * 100
