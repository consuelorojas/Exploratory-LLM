...
class TestDigitClassification:
    ...
    def test_model_loads(self):
        try:
            tf.keras.models.load_model(constants.MODEL_DIGIT_RECOGNITION_PATH)
        except Exception as e:
            raise AssertionError(f"Failed to load model: {e}")

def get_test_images() -> np.ndarray:
    # Load and preprocess the MNIST dataset or any other test images
    from tensorflow import keras
    (x_train, y_train), (_, _) = keras.datasets.mnist.load_data()
    
    x_train = x_train.reshape(-1, 28, 28)
    return x_train[:100]  # Use a subset of the data for testing

def get_expected_results() -> np.ndarray:
    from tensorflow import keras
    (x_train, y_train), (_, _) = keras.datasets.mnist.load_data()
    
    return y_train[:100]  # Use a subset of the labels for testing
